...
- **樱花 FRP 联机**：填自己的樱花 FRP 访问密钥，在可用节点上建一条 UDP 24642 隧道并拉起官方 frpc，把连接地址发给朋友，他们在游戏里按局域网联机进来
...
4. 想和朋友一起种地：到「樱花联机」填访问密钥 → 建一条 UDP 隧道 → 启动，把「复制地址」得到的连接地址发给朋友，他们在游戏里走「合作 → 加入局域网游戏 → 输入地址」
...
1. 到「樱花联机」页，把樱花 FRP 面板里的**访问密钥**填进来，点「验证并加载」
   - 免费账号也能开隧道；密钥等价于账号密码，只存在本机 `Data\Settings.json`
2. 点「准备 frpc」把官方客户端下载到本机（只下一次，带 MD5 校验）
3. 填隧道名 → 「加载节点」选一个节点 → 「创建 UDP 隧道（本地端口 24642）」
4. 在「我的 UDP 隧道」里点「启动」，等 frpc 连上节点后「复制地址」，把它发给朋友
5. 你在游戏里开合作模式，朋友走「合作 → 加入局域网游戏 → 输入这个地址」

> 隧道由樱花 FRP 的服务端与官方 frpc 维持，启动器只负责拉起进程、读取日志、把地址显示出来。用完后记得点「停止隧道」。
...
src/
  ...
  StardewLauncher.App/      WPF 界面
    Windows/    主窗口及各功能弹窗
    Pages/      启动 / Mod 管理 / 游戏实例 / 资源中心 / 樱花联机 / 设置
    Views/      嵌入页面的子视图与背景层
