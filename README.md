...
- **环境体检**（工具箱）：把启动器（含数据目录能不能写）、游戏目录、SMAPI、Mods 统计、依赖与版本要求、Mod 冲突、磁盘空间、日志与数据占用一次扫完；结论可以复制，也能连日志打成一个反馈包（Nexus 与樱花密钥不进包）
- **Mod 冲突检查**（工具箱）：找出多个 Mod 抢同一个 UniqueID、`manifest.json` 读不出来、以及放错层级导致 SMAPI 看不见的目录与散落文件
- **SMAPI 日志分析**（工具箱）：读最近一份 SMAPI 日志，按来源（SMAPI / 游戏本体 / 每个 Mod）统计错误与警告，把「谁在刷屏」排在前面，并列出被跳过的 Mod 与关键错误原文
- **原版文件还原**（工具箱）：被 XNB / Content 覆盖过的游戏原版文件可以从覆盖前备份还原；备份就地留在原文件旁边，索引记着是哪个包覆盖的，还原前还会把当前文件另存一份
...
> 备份一律放在启动器的 `Data\save-backups\` 里，**不往游戏存档目录里塞文件**，免得被 Steam 云同步带走。每个存档默认最多保留 10 份（超出自动删最旧的），回滚前默认自动快照 —— 这两项在「设置 → 存储与日志 → 存档备份」里调（对应 `Data\Settings.json` 里的 `SaveBackupKeepCount` 与 `SnapshotBeforeRestore`）。

### 工具箱里的另外四个工具

侧栏「工具箱」除了存档管理，还收了四个「偶尔要用」的工具，点卡片上的「打开」就在页内切进去：

| 工具 | 干什么 |
| --- | --- |
| **环境体检** | 一次扫完启动器（版本、运行时、数据目录能不能写）、游戏目录、SMAPI、Mods 统计、依赖与版本要求、Mod 冲突、磁盘空间、日志与数据占用、上次更新残留，每条给出「正常 / 提示 / 注意 / 问题」 |
| **Mod 冲突检查** | 重复 UniqueID（只算已启用的）、`manifest.json` 读不出来、Mods 下没有 `manifest.json` 的目录与直接丢在根目录的文件 |
| **SMAPI 日志分析** | 读最近一份 SMAPI 日志，按来源统计错误与警告并按数量排序，附被跳过的 Mod 与去重后的关键错误原文 |
| **原版文件还原** | 把被 Mod 覆盖过的游戏原版文件从覆盖前备份还原回去 |

几个约定：

1. **体检报告可复制**（纯文本，直接贴群里提问），也可以**导出反馈包** —— 一个 zip，含体检报告、Mod 清单、设置摘要与最近 5 份运行日志。**Nexus 密钥与樱花 FRP 密钥不会写进包里**：设置摘要只记录「有没有配置」，另外所有文本在落包前还会把密钥的实际值抹掉。
2. **冲突检查只看磁盘上已有的信息**，不解析内容包里的 `content.json`，所以"两个美化包抢同一张贴图"这类查不出来；那种情况看 SMAPI 日志分析或游戏里的实际表现。
3. **日志分析把异常堆栈的续行算作上一条错误的一部分**，不会每行堆栈都记一条错误，否则一个崩溃堆栈就能把某个 Mod 的错误数刷到几十条、排名就没意义了。日志位置按 SMAPI 官方的说法找：`%APPDATA%\StardewValley\ErrorLogs\` 下的 `SMAPI-crash.txt` 与 `SMAPI-latest.txt`，取最近改动的那一份。
4. **原版文件还原**的列表里有两类：**已登记**（索引里记着是哪个包覆盖的，能确定是原版）与**未登记**（索引之前留下的 `.bak-`，可能是原版也可能是上一个 Mod 覆盖后的版本，工具会明确标出来）。还原时会先把当前文件另存为 `<文件名>.before-restore-<时间戳>`，备份本身也保留，所以还原这一步同样能反悔。Mods 目录里的 `.bak-` 是旧版 Mod，不会被列出来。
5. 在「设置 → Mod 与下载 → 覆盖前备份」里可以关掉覆盖前备份（默认开）。关掉后不生成 `.bak-`，原版文件还原也就没有东西可还原了；Mod 文件夹的同名备份不受这个开关影响 —— 那只是把旧目录改个名，不复制、不占额外空间。
...
Data\game-backups\     覆盖前备份的索引（备份文件本身留在游戏目录里，贴在原文件旁边）
...
资产名要以 `StardewLauncher` 开头，并以 `.exe` 或 `.zip` 结尾（例如 `StardewLauncher.exe`、`StardewLauncher-v1.7.0-win-x64.zip`）——名字对不上就不会被当成更新包。
...
dotnet publish src\StardewLauncher.App\StardewLauncher.App.csproj ^
    ...
    -p:PublishSingleFile=true -p:DebugType=none -p:Version=1.7.0 -o publish
```
...
src/
  StardewLauncher.Core/     纯逻辑，不依赖 WPF
    ...
    Diagnostics/ 环境体检、SMAPI 日志分析、反馈包
...
**Q：装了 Mod 但游戏里没生效？**
A：三个常见原因——① 实例选的是「原版」而不是「Mod 端」；② 该 Mod 被禁用了（名字前缀是点号）；③ 缺前置，去「Mod 管理」跑一次依赖检查。还找不到就用「工具箱 → Mod 冲突检查」与「SMAPI 日志分析」。

**Q：游戏报错 / 闪退，不知道是哪个 Mod 干的？**
A：用 SMAPI 启动一次游戏（让日志生成），再到「工具箱 → SMAPI 日志分析」，它会按来源统计错误并排好序。要发出来求助就用「工具箱 → 环境体检」里的「导出反馈包」，日志、Mod 清单和体检报告都在里面。

**Q：「原版文件还原」里是空的？**
A：两种可能：① 还没发生过覆盖（只有会替换游戏原版文件的安装包才会留下备份）；② 已经在「设置 → Mod 与下载 → 覆盖前备份」里把它关掉了。另外 Mods 目录里的 `.bak-` 是旧版 Mod，不属于这里的范围。
