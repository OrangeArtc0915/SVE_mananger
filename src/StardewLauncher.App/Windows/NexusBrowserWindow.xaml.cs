...
/// <summary>
/// 在启动器内嵌的浏览器里浏览各 Mod 站点（默认 Nexus Mods，站点清单见 <see cref="ModSiteCatalog"/>）。
...
public partial class NexusBrowserWindow : Window
{
    ...
    private void HandleNxmUri(string uri, string source)
    {
        ...
        if (link is null)
        {
            ...
            MessageBox.Show(this, "无法解析该下载链接。", "在线下载 Mod",
        ...
        if (!link.IsStardewValley)
        {
            ...
            MessageBox.Show(this, "该链接不是星露谷物语的 Mod。", "在线下载 Mod",
        ...
        var dialog = new NxmDownloadWindow(link) { Owner = this };
        dialog.ShowDialog();
    }
